...
class SkillModels
{
    ...
    /**
     * Find a skill by name (case-insensitive), or create it with the given category.
     * Returns the skill row (id, name, category).
     */
    public function findOrCreate(string $name, string $category = ''): array
    {
        $stmt = $this->pdo->prepare('SELECT * FROM `skills` WHERE LOWER(name) = LOWER(?) LIMIT 1');
        ...
        $insert = $this->pdo->prepare('INSERT INTO `skills` (name, category) VALUES (?, ?)');
        $insert->execute([trim($name), trim($category)]);
        $newId = (int) $this->pdo->lastInsertId();

        return ['id' => $newId, 'name' => trim($name), 'category' => trim($category)];
    }
}